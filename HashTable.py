...
class HashTable:
    ...
    def search(self, value):
        ...
        while i < self.size:
            ...
            i = i + 1
            if self.table[key] is None or self.table[key] == "Deleted" or i >= self.size:
                end_s = time.perf_counter()
                time_array_search[n_s] = round(time_array_search[n_s - 1] + ((end_s - start_s) * 1000), 4)
    ...
    def delete(self, value):
        ...
        if key[0] is None:
            ...
            self.table[key[0]] = "Deleted"
            self.count -= 1
            return print(self.table[key[0]])

    def __str__(self):import math
import time
...
class HashTable:
    ...
    def __str__(self):
        ...
        items = [f"{value}" if value is not None else "None" for value in self.table]
        return "[" + ", ".join(items) + "]"
